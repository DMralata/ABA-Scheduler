...
import type { ProviderAvailability, DayOfWeek } from "@prisma/client";
import { setProviderAvailability } from "@/lib/actions/providers";
...
export function ProviderAvailabilityPanel({
  ...
  function saveDay() {
    ...
    setIsPending(true);
    setProviderAvailability(providerId, day, windows)
      .then((result) => {
        setIsPending(false);
        if (!result.success) {
