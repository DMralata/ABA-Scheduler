...
async function main() {
  ...
  const blockLabel = (b: Block) =>
    `${b.client.first}${b.client.lastInitial ? ` ${b.client.lastInitial}.` : ""} with ${b.providerFirst}${b.providerLastInitial ? ` ${b.providerLastInitial}.` : ""} ${b.dateStr} ${hhmm(b.startMins)}–${hhmm(b.endMins)}`;
  ...
  for (const b of blocks) {
    ...
    if (!provider || !client || !sessionType) {
      // Should not happen - every sheet person is matched or created above.
      // Log loudly instead of dropping the session silently.
      console.warn(`  ⚠ NOT CREATED (lookup failed${!provider ? ": provider" : !client ? ": client" : ": session type"}) — ${blockLabel(b)}`);
      continue;
    }
    ...
    if (overlap) {
      skippedOverlap++;
      // Same client at the same time = already imported (normal on re-runs).
      // A DIFFERENT client occupying the slot is the case worth shouting about.
      if (overlap.clientId !== client.id) {
        console.warn(`  ⚠ SKIPPED (provider already booked with a different client: "${overlap.name}") — ${blockLabel(b)}`);
      }
      continue;
    }
  ...
  // ── Verification: does the DB week now match the sheet? ────────────────────
  if (!DRY) {
    const weekStart = zonedTimeToUtc(WEEK!, 0, timezone);
    const weekEnd = zonedTimeToUtc(addDays(WEEK!, 7), 0, timezone);
    let missing = 0;
    for (const b of blocks) {
      const client = clientById.get(clientKey(b.client));
      if (!client) continue;
      const found = await prisma.session.findFirst({
        where: {
          ...
          AND: [
            { startTime: { lte: zonedTimeToUtc(b.dateStr, b.startMins, timezone) } },
            { endTime: { gte: zonedTimeToUtc(b.dateStr, b.endMins, timezone) } },
          ],
        },
        select: { id: true },
      });
      if (!found) {
        missing++;
        console.warn(`  ✗ VERIFY: no session in DB covering ${blockLabel(b)}`);
      }
    }
    console.log(missing === 0
      ? `\n✓ Verified: every block on the sheet is covered by a session in the database.`
      : `\n✗ ${missing} sheet block(s) have no covering session — see VERIFY lines above.`);
  }
